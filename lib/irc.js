...
function Client(server, clientNick, opt) {
    ...
    self.out.error = function() {
        if (!this.debug && !this.showErrors) return;
        ...
        switch (message.command) {
            ...
            case 'err_nicknameinuse':
                ...
                if (self.opt.autoRenick) {
                    var renickTimes = 0;
                    clearInterval(self.conn.renickInterval);
                    self.conn.renickInterval = setInterval(function() {
                        self.send('NICK', self.opt.nick);
                        renickTimes++;
                        if (self.opt.renickCount && renickTimes >= self.opt.renickCount) {
                            clearInterval(self.conn.renickInterval);
                            self.conn.renickInterval = null;
                        }

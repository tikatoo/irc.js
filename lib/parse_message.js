...
module.exports = function parseMessage(line, stripColors, enableStrictParse) {
    ...
    match = line.match(/^@([^ ]+) +/);
    if (match) {
        var tagsSource = match[1].split(';');
        line = line.substr(match[0].length)
        message.tags = {};

        for (let tagSource of tagsSource) {
            let sepIndex = tagSource.indexOf('=')
            let tagKey = tagSource.slice(0, sepIndex)
            let tagValue = tagSource.slice(sepIndex + 1)
                .replace(/\\(.)/g, (_, c) => {
                    switch (c) {
                    case ':':
                        return ';'
